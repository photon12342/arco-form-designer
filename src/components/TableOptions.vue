<template>
    <div class="_td-table-opt">
            ...
            <template v-for="(col,idx) in column" :key="col.label + idx">
                <a-table-column :label="col.label">
                    <template #default="scope">
                        <a-input :size="size || 'small'" :modelValue="scope.row[col.key] || ''"
                                  @Update:modelValue="(n)=>(scope.row[col.key] = n)"
                                  @blur="onInput(scope.row)"></a-input>
                    </template>
                </a-table-column>
            </template>
            <a-table-column width="70" align="center" fixed="right" :label="t('tableOptions.handle')">
                <template #default="scope">
        ...
        <div class="_td-table-opt-handle">
            <a-button link type="primary" @click="add" v-if="!max || max > value.length">
