<template>
    <div class="_fd-size-input">
        ...
        <template v-else>
            <a-inputNumber :size="size" v-model="num" @change="submit" controls-position="right"/>
            <a-dropdown trigger="click" size="small">
                <a-button :size="size">{{ unit[idx] }}</a-button>
                <template #content>
                    <a-doption v-for="(name, idx) in unit" :key="name" @click="changeType(idx)">
                        <div>{{ name }}</div>
                    </a-doption>
                </template>
            </a-dropdown>
...
._fd-size-input .arco-input-number-size-small {
    width: 122px;
}

._fd-size-input .arco-button {
    font-size: 14px;
    padding: 5px;
    margin-left: 3px;
    width: 25px;
