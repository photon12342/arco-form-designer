        ...
        <a-tree
            ...
            <template #default="{ node, data }">
                <div class="_fd-tree-opt-node">
                    <a-input class="_fd-tree-opt-first" v-model="data[overColumns.label]"
                              @blur="change"/>
                    <a-input class="_fd-tree-opt-last" v-model="data[overColumns.value]" @blur="change">
                        <template #append>
...
._fd-tree-opt .arco-tree-node {
    margin-bottom: 3px;
    height: 28px;
}

._fd-tree-opt .arco-input {
    border-right: 0 none;
}

._fd-tree-opt .arco-input-append {
    width: 90px;
    padding-right: 2px;
    padding-left: 1px;
    background: #fff;
