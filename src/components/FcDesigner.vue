	...
	>
		<a-layout-content>
			<a-layout style="height: 100%" :key="locale && locale.name">
				<a-layout-sider class="_fc-l" :width="leftSiderWidth">
					<a-layout style="height: 100%">
						<a-layout-header height="40px" class="_fc-l-tabs">
							<div
								class="_fc-l-tab active"
							>
								{{ t("menu.component") }}
							</div>
						</a-layout-header>
                ...
                <!-- <div class="split-panel-resizer" @mousedown="handlerResizer('left')"></div> -->
				<a-layout class="_fc-m">
						...
						<div class="_fc-m-tools-r">
							<template v-if="!inputForm.state">
								<a-space>
									...
									<a-button type="dashed" size="small" @click="openPreview"
										><i class="fc-icon icon-preview"></i>
										{{ t("props.preview") }}
									</a-button>
									<a-popconfirm
										:content="t('designer.clearWarn')"
										width="200px"
									...
									>
										<a-button class="_fd-m-extend" type="dashed" size="small">
					...
					<a-layout-content class="_fc-m-con">
						...
						<div class="_fc-m-input-handle" v-if="inputForm.state">
							<a-button type="dashed" @click="inputClear()">{{
								t("props.clear")
							}}</a-button>
							<a-button type="dashed" @click="inputReset()">{{
								t("props.reset")
							}}</a-button>
							<a-button type="dashed" @click="inputSave()">{{
								t("props.save")
							}}</a-button>
						</div>
					</a-layout-content>
				</a-layout>
                <div class="split-panel-resizer" @mousedown="handlerResizer('right')"></div>
				<a-layout-sider
					class="_fc-r"
				...
				>
					<a-layout style="height: 100%">
						...
						<a-layout-content
							class="_fc-r-tab-form _fc-r-tab-content"
						...
						<a-layout-content
							class="_fc-r-tab-props _fc-r-tab-content"
						...
						>
							...
							>
								...
								<a-tag color="green" size="small" bordered>
		...
		>
			...
			></ViewForm>
            <template #footer>
                <a-button size="small" @click="drawerVisible = false">关 闭</a-button>
            </template>
		</a-drawer>
	</a-layout>
</template>
        ...
        watch(
          ...
          data.validateForm.api.formData && data.validateForm.api.formData();
            ...
            data.propsForm.api.formData && data.propsForm.api.formData();
                    data.propsForm.rule = data.cacheProps[rule._fc_id] = tidyRuleConfig(
                        rule._menu.props,
        ...
        const methods = {
            ...
            handlerResizer(type) {
                document.onmousemove = function (e) {
                    ...
                    if(type === 'right') {
                        if(clientWidth - e.clientX < 200 || clientWidth - e.clientX > 500) {
                            return
                        }
                        data[`${type}SiderWidth`] = clientWidth - e.clientX;
                    } else {
                        if(e.clientX < 250 || e.clientX > 350) {
                            return
                        }
                        data[`${type}SiderWidth`] = e.clientX;
                    }
                };
          ...
          data.validateForm.api[data.activeRule._fc_id] !== data.activeRule
            ...
            toolActive(rule) {
                ...
                if (!data.cacheProps[rule._fc_id]) {
                    data.cacheProps[rule._fc_id] = tidyRuleConfig(
                        rule._menu.props,
                        componentRule.value && componentRule.value[rule._menu.name],
                        rule,
                        {
